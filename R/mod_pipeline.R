...
mod_pipeline_server <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    Additional_Initialize_Class <- function(){
      if(verbose) cat(paste0('::Additional_Initialize_Class() from - ', id, '\n\n'))
      
      rv.pipeline$data2send <- NULL
      rv.pipeline$tmp.return <- reactiveValues()
      rv.pipeline$child.process <- setNames(lapply(rv.process$config$steps,
                                            function(x){
                                              assign(x, base::get(paste0(rv.process$config$name, '_', x)))$new(ns(x))
                                            }),
    ...
    Set_All_Reset = function(){
      ...
      lapply(rv.process$config$steps, function(x){
        rv.pipeline$child.process[[x]]$Set_All_Reset()
      })
    }
    ...
    output$EncapsulateScreens <- renderUI({
      lapply(1:length(rv.process$config$steps), function(i) {
        if (i==1)
          ...
          shinyjs::hidden(
            div(id = self$ns(rv.process$config$steps[i]),
                class = paste0("page_", id),
                screens[[i]]
            )
          )
    ...
    GetScreens_ui = function(){
      if(verbose) cat(paste0('GetScreens() from - ', id, '\n\n'))
      
      setNames(lapply(rv.process$config$steps, function(x){
        rv.pipeline$child.process[[x]]$ui()
      }),
      rv.process$config$steps)
    }
    ...
    Launch_Module_Server = function(){
      ...
      lapply(rv.process$config$steps, function(x){
        rv.pipeline$tmp.return[[x]] <- rv.pipeline$child.process[[x]]$server(
          dataIn = reactive({ rv.pipeline$data2send[[x]] })
        )
      })
      
      # Catch the returned values of the process                                                           
      observeEvent(lapply(names(rv.pipeline$child.process), function(x){rv.pipeline$tmp.return[[x]]()$trigger}), ignoreInit=T,{
